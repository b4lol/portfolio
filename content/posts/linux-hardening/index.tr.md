...
Bunu söylemekle birlikte, Linux size bütün bu sorunları çözecek araçları sunar. Sadece bunları siz yapılandırmanız gerekir. Bu kılavuzda yapacağımız şey de tam olarak budur.

## Güvenli bir Linux dağıtımı seçmek {#scelta-distro style="color: white;"}

Dağıtım seçimi her şeyin temelidir. Burada yapılan yanlış bir seçim, sonraki tüm çalışmayı anlamsız kılabilir. Gerçekten önemli olan kriterlere bakalım.
...
**Önerim**: sık güncellemelerden çekiniyorsanız, rollback mekanizmaları sunan bir dağıtım seçin (dnf history ile Fedora veya Btrfs snapshot ile openSUSE gibi). Böylece her iki dünyanın en iyisini elde edersiniz: güncel paketler ve bir şey bozulursa geri dönebilme imkânı.
...
Düzinelerce dağıtımı analiz ettikten sonra, farklı kullanıcı profillerine göre önerdiklerim şunlar.
...
Fedora denge ise, Debian sağlamlıktır. En iyi anlamıyla "sıkıcı" bir sistemdir: kurarsınız, çalışır ve minimum bakımla yıllarca çalışmaya devam eder. Sürekli elini sokmak zorunda kalmadan güvenilir bir masaüstü isteyen herkese önerilebilecek, güvenli ve her zaman kullanıma hazır temeldir.
...
Upstream düzeltmelerindeki gecikmeyi yumuşatmak için en açık uygulamalarınızı (özellikle tarayıcıyı) Flatpak ile kurun: bu şekilde Debian'ın sürüm döngüsünden bağımsız olarak güncel kalırlar, temel sistem de aradığınız kararlılığı sağlar.
...
Manuel yapılandırma çabasını en aza indirip maksimum güvenlik isteyenler için önerdiğim dağıtım budur. Tek dezavantajı, Fedora'ya ek olarak bir projeye daha güvenmeniz gerekmesidir.
...
- En aşırı güvenlik gereksinimlerine sahip kişiler için önerilen sistemdir

**Eksileri:**
- Önemli donanım gereksinimleri: çok fazla RAM (16 GB makul minimum) ve sanallaştırma destekli bir CPU gerekir
- Dik öğrenme eğrisi: qube modeli bir zihniyet değişikliği gerektirir
...
- **Kali Linux, BlackArch, Parrot OS**: bunlar penetrasyon testi araçlarıdır, günlük kullanım için *güvenli* sistemler değildir. Önceden kurulu saldırı araçlarına sahiptir ve genellikle root olarak çalışır. Bunları günlük masaüstü olarak kullanmak, markete gitmek için ambulans kullanmak gibidir
- **Linux-libre çekirdekli dağıtımlar**: tescilli microcode için güvenlik azaltımlarını kaldırır ve CPU güvenlik açığı uyarılarını bastırır. İdeolojik nedenlerle gerçek güvenliğinizi feda ederler
- **Manjaro**: gerçek bir kararlılık avantajı olmadan paketleri Arch'a kıyasla geride tutar, gereksiz bir güvenlik açığı penceresi oluşturur

## Disk şifreleme ile güvenli Linux kurulumu {#installazione style="color: white;"}

Tamam, dağıtımınızı seçtiniz. Şimdi onu baştan doğru şekilde nasıl kuracağımıza bakalım. Bu yapılandırmalardan bazıları sonradan yapılamaz, bu yüzden gözler açık ve azami dikkat!

### Tam disk şifreleme (LUKS)

Kurulum sırasında tam disk şifrelemeyi etkinleştirmek esastır. LUKS (Linux Unified Key Setup) ile diskinizdeki tüm veriler şifrelenir: biri laptop'ınızı çalarsa, parola olmadan hiçbir şey okuyamaz.

**DİKKAT!** Kurulum sırasında şifrelemeyi etkinleştirmezseniz, tüm verilerinizi yedeklemeniz ve sıfırdan yeniden kurulum yapmanız gerekecek. Zaten kullanımda olan bir diski veri kaybı olmadan şifrelemek mümkün değildir.

Kurulum sırasında, bölümlendirmeye geldiğinizde:
...
Maksimumu isteyenler için: kimlik doğrulamalı şifreleme, verilerin sadece okunamaz olduğunu değil, üzerinde oynama yapılmadığını da doğrular. Sofistike fiziksel saldırılara karşı ek bir koruma katmanıdır.
...
> **Dikkat!** `noexec`'i `/home` veya `/root`'a eklemeyin çünkü Flatpak ve Snap'i bozar. Aynı şekilde, Arch kullanıyorsanız `/var/tmp` üzerinde `noexec`'ten kaçının (AUR derlemeleri başarısız olur).

Bu seçenekler kusursuz değildir (`noexec` nispeten kolay aşılabilir), ama otomatikleştirilmiş saldırıları engelleyebilen bir derinlemesine savunma katmanı eklerler.

### Genel kullanıcı adı ve hostname

Çoğu kişinin gözden kaçırdığı bir ayrıntı: kullanıcı adınız ve hostname'iniz ağ üzerinde çeşitli şekillerde iletilir ve sizi tanımlamak için kullanılabilir.
...
Kurulum tamamlandı, disk şifrelendi, bölümler yapılandırıldı. Şimdi gerçek sertleştirme çalışması başlıyor.
...
Sonra CPU microcode'unun kurulu olduğundan emin olun. Microcode, hata ve güvenlik açıklarını doğrudan işlemcide düzelten firmware'dir. Bu olmadan, bir dizi donanım saldırısına (Spectre, Meltdown ve benzerleri) karşı güvenliksiz kalırsınız.
...
Birçok cihazda güvenlik düzeltmeleri içerebilen güncellenebilir firmware bulunur. `fwupd` kullanın:
...
Bazı dağıtımlar kullanım hakkında anonim veri toplar. Niyetler iyi olsa da, ne kadar az veri paylaşırsanız o kadar iyi:
...
> **Dikkat!** openSUSE'de bu Snapper'ı bozabilir. Ubuntu'da, `/etc/apt/sources.list.d/` içindeki depo dosyalarının 600 değil 644 izinlere sahip olması gerekir. Uyguladıktan sonra kontrol edin.

## Ağ sertleştirme: güvenlik duvarı, DNS ve MAC randomizasyonu {#rete style="color: white;"}

Ağ en yaygın saldırı vektörlerinden biridir. Nasıl güçlendirileceğine bakalım.

### MAC adresi randomizasyonu

MAC adresi, ağ kartınızın benzersiz bir tanımlayıcısıdır. Bir WiFi'ye her bağlandığınızda, router bunu görür ve mevcudiyetinizi izleyebilir. Randomize ederek, her bağlantı farklı bir cihaz olarak görünür.
...
Bir güvenlik duvarı kesinlikle zorunludur. Önerdiğim yapılandırma kısıtlayıcıdır: açıkça izin verilmedikçe tüm gelen trafiği engelleyin.
...
`ufw` firewalld'den daha basit ama daha az esnektir. Bölgeleri desteklemez ve lockdown modu yoktur. Bir masaüstü için yine de fazlasıyla yeterlidir.



**Unutmayın ki** bir yazılım güvenlik duvarı, sisteminizde yükseltilmiş ayrıcalıklarla çalışan kötü amaçlı yazılıma karşı koruma sağlayamaz. Bir savunma katmanıdır, eksiksiz bir çözüm değildir.

### DNSSEC

Geleneksel DNS açık metin ve kimlik doğrulamasızdır: sizinle DNS sunucusu arasındaki yoldaki herkes sorgularınızı görebilir ve yanıtları potansiyel olarak değiştirebilir. DNSSEC, DNS yanıtlarına kriptografik bir imza ekler.
...
DNSSEC'i destekleyen bir DNS sağlayıcısı kullandığınızdan emin olun. Ek bir gizlilik katmanı için DNS-over-TLS veya DNS-over-HTTPS kullanmayı düşünün.

### Güvenli zaman senkronizasyonu

Zaman senkronizasyonu için standart protokol olan NTP, açık metin ve kimlik doğrulaması olmadan iletim yapar. Bir saldırgan sisteminizin saatini manipüle ederek TLS sertifikalarını ve günlükleri tehlikeye atabilir.

Çözüm, chronyd ile **Network Time Security (NTS)**'dir:
...
`minsources 2` parametresi, en az 2 bağımsız kaynağın saat üzerinde anlaşmasını gerektirir, bu da bir saldırıyı çok daha zor hale getirir.
...
Çekirdek, işletim sisteminin kalbidir. Onu sertleştirmek, potansiyel kötü amaçlı yazılım için mevcut saldırı yüzeyini büyük ölçüde azaltmak demektir.

### sysctl parametreleri

sysctl parametreleri çekirdeğin çalışma zamanındaki davranışını kontrol eder. Aşağıdaki temel yapılandırmaları içeren bir `/etc/sysctl.d/99-hardening.conf` dosyası oluşturun:
...
Bu parametrelerin temel ihtiyaçları karşıladığını söyleyelim. Daha eksiksiz bir yapılandırma için, workstation'lar için güncel bir sysctl yapılandırması sürdüren [TommyTran732'nin deposuna](https://github.com/TommyTran732/Linux-Setup-Scripts/blob/main/etc/sysctl.d/99-workstation.conf) başvurabilirsiniz.
...
**DİKKAT!** SMT'yi (Simultaneous Multi-Threading / Hyper-Threading) devre dışı bırakmanın performans üzerinde önemli bir etkisi vardır. Yoğun iş yükleriyle (derleme, video düzenleme, oyun) çalışıyorsanız, biraz daha yüksek bir riski kabul ederek `nosmt=force`'u kaldırmak isteyebilirsiniz.
...
Bunlar donanım cihazlarından (Thunderbolt veya PCIe gibi) gelen DMA saldırılarına karşı korur. Eksiksiz bir koruma sunmadıklarını unutmayın: early boot sırasındaki bir saldırı, IOMMU aktif olmadan önce çekirdeği yine de ele geçirebilir.
...
Kullanmıyorsanız devre dışı bırakılması gereken en önemli modüller:
...
> **Dikkat!** `hfsplus`'ı devre dışı bırakmadan önce, EFI bölümünüzün dosya sistemini kontrol edin. HFS+ ise, devre dışı bırakmak boot'u engeller. `df -T /boot/efi` ile kontrol edin.
...
Geleneksel bir Linux masaüstünde, her uygulama neredeyse her şeye erişebilir: dosyalarınıza, ağa, çevre birimlerine, çalışmakta olan diğer uygulamalara. Sandboxing bu erişimi gerekli olan minimuma indirir.

### Flatpak: önerilen seçenek

Flatpak, Linux masaüstünde sandboxing için en olgun uygulama dağıtım sistemidir. Her uygulama açık izinlerle bir sandbox içinde çalışır.
...
Önce tüm uygulamalara kısıtlayıcı bir politika uygulayın, sonra gerektiğinde belirli izinler verin:
...
**Strateji şu**: önce her şeyi geri alın, sonra uygulamanın çalışıp çalışmadığını test edin. Çalışmıyorsa, gereken minimumu bulana kadar tek tek izin verin.

#### Flatseal: görsel izin yönetimi

Komut satırıyla çıldırmadan izinleri yönetmek için Flatseal'ı kurun:
...
**DİKKAT!** Denetimsiz otomatik Flatpak güncellemelerini etkinleştirmeyin. Bir uygulama güncellendiğinde, yeni izinler bildirim olmadan otomatik olarak verilir. Manuel olarak güncelleyin ve changelog'ları kontrol edin.
...
Bana göre, Flatpak kullanabiliyorsanız onu tercih edin. Firejail'i yalnızca Flatpak olarak mevcut olmayan uygulamalar için kullanın.
...
Geleneksel masaüstü dağıtımlarında MAC'in yalnızca bazı sistem daemon'larını sınırladığını, tüm uygulamaları değil, unutmayın. Önemli ama eksiksiz olmayan bir savunma katmanıdır.

## Secure Boot ve sistemin fiziksel güvenliği {#sicurezza-fisica style="color: white;"}

Biri makinenize fiziksel olarak erişip onunla oynayabiliyorsa, dünyadaki tüm yazılım sertleştirmesi işe yaramaz.

### Özel anahtarlarla Secure Boot

Standart Secure Boot, bootloader ve çekirdeğin güvenilir anahtarlarla (genellikle Microsoft'un) imzalandığını doğrular. Sorun şu ki, Microsoft'un anahtarları üçüncü taraf sürücüleri ve bootloader'ları imzaladığı için çok büyük bir saldırı yüzeyine sahiptir.



**Arch Linux**'ta, **sbctl** ile kendi kişisel anahtarlarınızı kaydedebilirsiniz:
...
Fedora'da prosedür farklı yollar ve paketleme gerektirir: benzer adımları takip etmeden önce dağıtımınızın güncel dokümantasyonunu kontrol edin.

**DİKKAT!** Bu prosedür, uyumlu olmayan bazı UEFI uygulamalarını tuğlaya (brick) çevirebilir. İlerlemeden önce özel donanımınız hakkında araştırma yapın. Yedek olarak bir EEPROM yeniden programlama yöntemi hazırlayın.

### Unified Kernel Image (UKI)

Bir UKI, çekirdeği, initramfs'i ve microcode'u tek bir imzalı imajda birleştirir. Bu, initramfs'in üzerinde oynama yapılmasını önler (standart Secure Boot'un kapsamadığı bir saldırı vektörü).
...
SSH aktifse (mesela masaüstünüze uzaktan erişim için), onu doğru şekilde yapılandırmak esastır. `/etc/ssh/sshd_config`'i düzenleyin:
...
Kimlik doğrulama için **sadece SSH anahtarları** kullanmanızı (parolaları tamamen devre dışı bırakarak) şiddetle öneririm. Bir anahtar çifti oluşturmak için:
...
> **Dikkat!** pam-u2f'i yapılandırırken, ArchWiki dokümantasyonunda açıklandığı gibi `origin` ve `appid` için her zaman sabit kodlanmış (hardcoded) değerler kullanın. Varsayılan `pam://$HOSTNAME` değerlerini kullanmayın, çünkü hostname'i değiştirirseniz giriş çalışmayı durdurur.
...
Genellikle göz ardı edilen bazı temel kurallar:

- **Doğrudan root kullanmayın**: ayrıcalıklı komutlar için her zaman `sudo` kullanın
...
Çalışan her hizmet potansiyel bir giriş noktasıdır. İlke basit: kullanmıyorsanız, kapatın.
...
Tanımadığınız dinleyen hizmetler görürseniz, devre dışı bırakmadan önce araştırın.

## Wayland ile X11: ekran sunucusu güvenliği {#wayland style="color: white;"}

Daha önce bahsedildiği gibi, Wayland artık çoğu modern dağıtımda varsayılan grafik protokolüdür. Bu, güvenlik için büyük bir adımdır, çünkü eski X11'de pencereler arasında izolasyon kavramı hiç yoktu.

Bununla birlikte, sisteminizde hâlâ **XWayland** aktif olabilir. XWayland, eski X11 uygulamalarının Wayland altında çalışmasını sağlayan bir uyumluluk katmanıdır. Sorun şu ki, X11'in güvenlik kusurlarının bir kısmını yeniden ortaya çıkarır: XWayland altında çalışan uygulamalar, diğer XWayland uygulamalarının (ama yerel Wayland uygulamalarının değil) girişini ve ekranını potansiyel olarak yakalayabilir.
...
Çoğu kullanıcı için mevcut durum zaten iyidir: ana uygulamalar (tarayıcılar, dosya yöneticileri, terminaller, editörler) hepsi yerel Wayland'i destekler. Hâlâ XWayland gerektiren az sayıda uygulama genellikle daha eski veya özel olanlardır.
...
Güvenli bir sistemin aynı zamanda gözlemlenebilir olması gerekir. Bir şeyler ters giderse, günlükler ne olduğunu anlamak için tek bilgi kaynağınızdır.

### journald yapılandırması

Systemd journal, varsayılan günlükleme sistemidir. Günlüklerin yeniden başlatmalar arasında kalıcı olacak şekilde yapılandırıldığından emin olun:
...
Başardınız, harika kahramanlar! 🛡️

Standart bir Linux kurulumunu şunlarla sertleştirilmiş bir sisteme dönüştürdünüz:
...
Unutmayın: güvenlik bir varış noktası değil, bir yolculuktur. Sistemi güncel tutun, günlükleri düzenli olarak kontrol edin, yeni yazılım eklediğinizde yapılandırmaları gözden geçirin. Ve her şeyden önce, öğrenmeyi asla bırakmayın.

Okuduğunuz için çok teşekkürler! Bu kılavuz size yardımcı olduysa, fayda sağlayabileceğini düşündüğünüz herkesle paylaşın. Ne kadar çok bilinçli kaplumbağa olursa, herkes için deniz o kadar güvenli olur.
